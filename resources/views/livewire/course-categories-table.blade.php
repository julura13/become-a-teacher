<div>
    <div class="tw-w-full tw-flex tw-pb-10">
        <div class="relative">
            <input wire:model.debounce.300ms="search" type="search" class="tw-block tw-mx-2 tw-bg-gray-200 tw-text-gray-700 border tw-border-gray-200 tw-rounded tw-py-3 tw-px-4 tw-leading-tight focus:tw-outline-none focus:tw-bg-white focus:tw-border-gray-500"placeholder="Search categories...">
        </div>
        ...
        <table class="tw-table-fixed border">
            ...
            <tbody>
                @foreach($categories as $category)
                    <tr>
                        <td class="tw-truncate border tw-px-4 tw-py-2">{{ $category->id }}</td>
                        <td class="tw-truncate border tw-px-4 tw-py-2">{{ ucfirst($category->name) }}</td>
                        <td class="tw-truncate border tw-px-4 tw-py-2">{{ $category->created_at->diffForHumans()  }}</td>
                        <td class="tw-truncate border tw-px-4 tw-py-2">{{ $category->updated_at->diffForHumans()  }}</td>
                        <td class="tw-truncate border tw-px-4 tw-py-2 tw-text-center">
                            <a href="course/category/{{ $category->id }}" ><i class="ik ik-edit-2 f-16 mr-15 text-green"></i></a>
                            <a href="course/category/delete/{{ $category->id }}"><i class="ik ik-trash-2 f-16 text-red"></i></a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
